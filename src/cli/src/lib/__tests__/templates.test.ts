...
import { buildClaudeMd } from '../templates.js';
...
describe('buildClaudeMd', () => {
  ...
  it('appends SSH section when hasSshKey is true', () => {
    const result = buildClaudeMd(role, config, workspaceRoot, [], true);
    expect(result).toContain('SSH Authentication');
  });

  it('does not append SSH section when hasSshKey is false', () => {
    const result = buildClaudeMd(role, config, workspaceRoot, [], false);
  ...
  it('appends repository context when repos are provided', () => {
    ...
    const result = buildClaudeMd(role, config, workspaceRoot, repos);
